...
class DtakMakerTester(unittest.TestCase):
    ...
    @unittest.skip("From previous stage. Loading is now implicitly tested by test_verify_properties") 
    ...
    def test_verify_data_integrity(self) -> None:
        for i,example in enumerate(self.dataset):
            ...
            # Recreate: tok,ws -> raw
            orig_raw_from_tok = self.maker._modifier._tok2raw(example["orig_tok"], example["orig_ws"])
            assert example["orig"] == orig_raw_from_tok
            norm_raw_from_tok = self.maker._modifier._tok2raw(example["norm_tok"], example["norm_ws"])
            ...
            tok, xlit, lemma, pos, ws, spans = example["orig_tok"], example["orig_xlit"], example["orig_lemma"], example["orig_pos"], example["orig_ws"], example["orig_spans"]
            assert len(tok) == len(xlit) == len(lemma) == len(pos) == len(ws) == len(spans)
            
            # All norm annotations have same length
            tok, ws, spans = example["norm_tok"], example["norm_ws"], example["norm_spans"]
    ...
    def test_print_dtak_alignments(self) -> None:
        ...
        for i,example in enumerate(self.dataset.filter(lambda example: example["basename"].startswith("weigel"))):
            for aligned_pair in example["alignment"]:
                print(f"{example['orig_tok'][aligned_pair[0]]} <-> {example['norm_tok'][aligned_pair[1]]}")
            print()
            if i > 20:
                break
